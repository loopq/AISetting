---
name: plan-review-new
description: Use when the user says "/plan-review-new" and provides a plan file path that needs critical review and iterative refinement with Codex. Same adversarial plan-iteration workflow as plan-review, but Codex is invoked through the codex-plugin-cc companion runtime instead of ask_codex.sh.
---

# Plan Review Skill (codex-plugin-cc edition)

## Purpose

When the user runs `/plan-review-new {plan-file-path}`, start the "adversarial plan iteration" workflow:
...
```
/plan-review-new plans/my-feature-plan.md
```

## Codex Runtime

All Codex calls go through the codex-plugin-cc companion script. Resolve its path once at the start of the task:

```bash
COMPANION=$(find ~/.claude/plugins/cache/openai-codex -name codex-companion.mjs 2>/dev/null | sort -V | tail -1)
```

If `$COMPANION` is empty, the plugin is not installed. Stop and tell the user to run:
`claude plugin marketplace add openai/codex-plugin-cc && claude plugin install codex@openai-codex`
(or fall back to the legacy `/plan-review`).

**Invocation pattern** — always pass the prompt via heredoc stdin (avoids shell-quoting issues with markdown prompts):

```bash
# Round 1 (fresh thread):
node "$COMPANION" task --write <<'PROMPT_EOF'
{review prompt}
PROMPT_EOF

# Round 2+ (same thread, Codex remembers prior rounds):
node "$COMPANION" task --write --resume-last <<'PROMPT_EOF'
{review prompt}
PROMPT_EOF
```
...
- Do NOT pass `--effort` — reasoning effort inherits `model_reasoning_effort` from `~/.codex/config.toml` (currently `xhigh`), which is the desired behavior. Never use `--effort minimal` (conflicts with the enabled image_gen/web_search tools, API 400).
- Run with Bash `timeout: 600000` — reviews take several minutes.
- If the Bash call times out, do NOT re-run the task. The companion tracks jobs persistently: run `node "$COMPANION" status` to find the job, then `node "$COMPANION" result <job-id>` to recover the output.
- `--write` gives Codex workspace write access so it can append the review file itself, same as the legacy flow.
- `--resume-last` resumes the most recent task thread. Therefore: do NOT interleave any other codex task/rescue calls between review rounds of the same plan, or the thread anchor will point at the wrong conversation.
- Use the same full review prompt every round; `--resume-last` plus the review file's previous rounds give Codex the iteration context.
...
Invoke the companion `task` runtime (see **Codex Runtime** above) with the following prompt:
...
- If a local copy path is present, **read that file** and use it to validate the plan against the original requirement.
...
| Status           | My Action                                                                                                    |
| ---------------- | ------------------------------------------------------------------------------------------------------------ |
| `NEEDS_REVISION` | Revise the plan, then automatically ask Codex to review again (with `--resume-last`) and return to Step 2    |
